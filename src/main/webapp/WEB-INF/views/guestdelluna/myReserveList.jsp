...
<c:forEach var="exList" items="${listExp}" varStatus="status">
	...
	<tr>
		<td>
			<input type="checkbox" name="expCheck" class="expCheck" value="${exList.exReserveCode}"/>
		</td>
		<td>
			<div>번호</div>
	...
	<input type="hidden" name="exReserveCode" value="${exList.exReserveCode}"/>
	</table>
...
<c:forEach var="exList" items="${listExp}" varStatus="status">
	...
	<tr>		
		<td>
			<div>체험날짜</div>
			<div><fmt:formatDate value="${exList.reserveDate }" pattern="yyyy-MM-dd"/></div>
		</td>
...
<c:forEach var="houseList" items="${listHouse}" varStatus="status">
	...
	<tr>
		<td>
			<input type="checkbox" name="houseCheck" class="houseCheck" value="${houseList.reserveCode}"/>
		</td>
		<td>
			<div>번호</div>
			<div class="seqRes">${status.count}</div>
	...
	<input type="hidden" name="houseReserveCode" value="${houseList.reserveCode}"/>
...
<c:forEach var="houseList" items="${listHouse}" varStatus="status">	
	<table>
		...
		<td>
			<div>예약날짜</div>
			<div class="resDate"><fmt:formatDate value="${houseList.reserveDate}" pattern="yyyy-MM-dd"/></div>
		</td>
		
		<td>
			<div>예약여부</div>
			<di>${houseList.state}</div>
		</td>
	</tr>
	</table>
