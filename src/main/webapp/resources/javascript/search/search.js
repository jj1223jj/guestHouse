...
function setExList(ex, memberCode){
	for(let i=0;i<ex.length;i++){
		...
		exContent+=
			'					</div>' +
			'				</div>' +
			'				<a href="'+root+'/experience/exPage.do?exCode='+ex[i].exCode+'" style="display:block;">' +
			'					<div class="_houseInfo">' +
			...
			'						<div class="_houseFacilities" style="margin-top: 0.75rem">인원 '+ex[i].people+'명 · 침대 1개 · 욕실 1개</div>' +
			'						<div class="_houseFacilities" style="margin-top: 0.25rem">시작시간 '+ex[i].exStartDate+' · 종료시간 '+ex[i].exEndDate+' · 진행시간 '+ex[i].exTime+' · 주방 </div>' +
